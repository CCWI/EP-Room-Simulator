...
//const urlDefault = "https://www.ladybug.tools/spider-2020/spider-idf-viewer/idf-sample-files/theoretical-office-building-tel-aviv-2020-06-15.zip" //use if you want the ladybug spider sample building
const urlDefault = "http://localhost:100/downloadIDF" //use if you want idf from tool
//const urlDefault = "http://localhost:100/downloadIDF.zip"  //use if you want a zip version of idf from tool
//const urlDefault = path + files[ 16 ];
...
ItoJ.checkZones = function( group = THR.group ) {

	const index = ItoJ.parents.keys.indexOf( "  Zone" )
	//console.log( "index", index,  ItoJ.parents.values[ index + 1]);
	...
	const names = zonesArr.map( zone => zone[ 1 ].split( "," ).shift().trim() );
	...
	for ( let i = 0; i < deltas.length; i++ ) {
		...
		for ( let child of group.children ) {
			...
			for ( let child2 of group.children ) {
	...
	const multiplierArray = zonesArr.map( zone => {
		...
		zoneLine = zone.find( line => line.includes( "Multiplier" ) );

		return  zoneLine.trim().split( "," ).shift();
	...
	for ( let i = 0; i < multiplierArray.length; i++ ) {
		...
		const multiplier = + multiplierArray[ i ];
		...
		for ( let j = 0; j < multiplier; j++ ) {
			...
			const group = zoneGroup.clone();
			group.position.z = j * height;
			THR.group.add( group );
		}
